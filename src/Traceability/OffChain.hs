...
-- | mintLC mints traceability tokens and increments the traceability counter
...
mintLCToken tt tp = do
    ...
    Contract.logInfo $ "mintLCToken: found traceability utxo with datum= " ++ Haskell.show lcd
    Contract.logInfo $ "mintLCToken: found traceability utxo oref= " ++ Haskell.show orefLC
    Contract.logInfo $ "mintLCToken: traceability hash= " ++ Haskell.show (lcHash $ PlutusTx.toBuiltinData lcParams)
        ...
        oref : _ -> do
            let red = Scripts.Redeemer $ toBuiltinData $ MintPolicyRedeemer 
                ...
                mintParams = LCMintPolicyParams 
                    {
                        lcTokenName = tn -- the name of the traceability
                    ,   lcAdminPkh = tpAdminPkh tp  -- the admin pkh who can only mint traceabilitys
...
-- | burnLC burns traceability tokens.   This offchain function is only used by the PAB
--   simulator to test the validation rules of the minting policy validator.  
...
burnLCToken tt tp = do
    ...
    (orefLC, oLC, lcd@LCDatum{}) <- findLCValidator lcParams threadTokenCurSymbol tt
    Contract.logInfo $ "burnLCToken: found traceability utxo with datum= " ++ Haskell.show lcd
    Contract.logInfo $ "burnLCToken: found traceability utxo oref= " ++ Haskell.show orefLC
    Contract.logInfo $ "burnLCToken: traceability hash= " ++ Haskell.show (lcHash $ PlutusTx.toBuiltinData lcParams)
        ...
        oref : _ -> do
            let tn = Value.TokenName $ tpNFTTokenName tp
                ...
                mintParams = NFTMintPolicyParams 
                    {
                        nftTokenName = tn -- the name of the traceability
...
-- | Initialize the traceability contract
...
-- | Find the traceability validator onchain using the lc params and threadtoken
...
-- | Add Ada to the traceability smart contract  This offchain function is only used by the PAB
--   simulator to test the validation rules of traceability validator. 
...
addAdaToContract tt tp = do
    ...
    Contract.logInfo $ "addAdaContract: found traceability utxo with datum= " ++ Haskell.show lcd
    Contract.logInfo $ "addAdaContract: found traceability utxo oref= " ++ Haskell.show oref
    Contract.logInfo $ "addAdaContract: traceability hash= " ++ Haskell.show (lcHash $ PlutusTx.toBuiltinData lcParams)
