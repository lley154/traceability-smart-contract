...
-- | LCDatum is used to record the amount of traceability minted and the amount
--   of Ada locked at the smart contract.  This is then used during traceability
--   burning to payout the corresponding amount of Ada per traceability to the merchant.
...
-- | The mktraceabilityPolicy is the minting policy validator for minting and burning
--   of traceability tokens.  It is a parameterized validator so each policy
--   id is unqiue based on the admin pkh provided.

{-# INLINABLE mktraceabilityPolicy #-}
mktraceabilityPolicy :: LCMintPolicyParams -> MintPolicyRedeemer -> ScriptContext -> Bool
mktraceabilityPolicy params (MintPolicyRedeemer polarity withdrawAmount) ctx = 
  ...
  where
    ...
    -- Check for NFT Merchant token if burning traceability
...
lcPolicy mpParams = mkMintingPolicyScript $
    $$(PlutusTx.compile [|| \mpParams' -> TScripts.wrapMintingPolicy $ mktraceabilityPolicy mpParams' ||])
    `PlutusTx.applyCode`
...
--   When a merchant has one of a merchant approved NFT, they are authorized to spend/burn traceability.
...
--   from the traceability smart contract.  
...
mkLCValidator params dat red ctx = 
    case red of
      ...
      where        
        ...
        -- | Check that the traceability token name minted is equal to the amount in the redeemer
        ...
        -- | Check that the traceability token name burned is equal to the amount in the redeemer
        ...
        -- | Check that the Ada spent matches traceability burned and that the
...
-- | Mint a unique NFT representing a traceability validator thread token
